        ...
        <div className="max-w-4xl mx-auto">
          <div className="text-center mb-8">
            <Link to="/" className="brand-logo text-4xl inline-block">
              Dream<span className="gold-text">pick</span>
            </Link>
            <div className="overline text-[#D4A93A] mt-3">Register</div>
            <p className="text-white/60 text-sm mt-2">Join Dreampick Private Limited. Activation requires manual admin confirmation.</p>
          </div>
          ...
          <div className="grid lg:grid-cols-5 gap-6">
            <form onSubmit={handleSubmit} className="lg:col-span-3 dp-card p-8 space-y-4">
              <div className="grid md:grid-cols-2 gap-4">
                <div>
                  ...
                  <Input required value={form.full_name} onChange={(e) => setField("full_name", e.target.value)}
                    className="mt-2 bg-[#0F0A1F] border-white/10 h-11" data-testid="register-fullname-input" />
                </div>
                <div>
                  ...
                  <Input required value={form.phone} onChange={(e) => setField("phone", e.target.value)}
                    className="mt-2 bg-[#0F0A1F] border-white/10 h-11" placeholder="+91-9000000000" data-testid="register-phone-input" />
                </div>
              </div>
              <div>
                ...
                <Input required type="email" value={form.email} onChange={(e) => setField("email", e.target.value)}
                  className="mt-2 bg-[#0F0A1F] border-white/10 h-11" data-testid="register-email-input" />
              </div>
              <div className="grid md:grid-cols-2 gap-4">
                <div>
                  ...
                  <Input required type="password" value={form.password} onChange={(e) => setField("password", e.target.value)}
                    className="mt-2 bg-[#0F0A1F] border-white/10 h-11" data-testid="register-password-input" minLength={6} />
                </div>
                <div>
                  ...
                  <Input required type="password" value={form.confirm_password} onChange={(e) => setField("confirm_password", e.target.value)}
                    className="mt-2 bg-[#0F0A1F] border-white/10 h-11" data-testid="register-confirm-password-input" minLength={6} />
                </div>
              </div>
              ...
              <div className="border-t border-white/5 pt-4">
                ...
                <Input value={form.referral_code} onChange={(e) => setField("referral_code", e.target.value.toUpperCase())}
                  className="mt-2 bg-[#0F0A1F] border-white/10 h-11 font-mono" placeholder="Optional referral code" data-testid="register-referral-input" />
                {form.referral_code && refInfo && (
                  <div className="mt-2 flex items-center gap-2 text-xs" data-testid="register-referral-status">
                    {refInfo.valid ? (
                      <><CheckCircle2 className="w-4 h-4 text-[#F4D06F]" /> <span className="text-[#F4D06F]">Valid — {refInfo.referrer.full_name} ({refInfo.referrer.user_code})</span></>
                    ) : (
                ...
                {refInfo?.valid && (
                  <div className="mt-4">
                    ...
                    <RadioGroup value={form.placement_side} onValueChange={(v) => setField("placement_side", v)} className="grid grid-cols-2 gap-3 mt-2">
                      <label htmlFor="left" className={`dp-card p-4 cursor-pointer ${form.placement_side === "LEFT" ? "border-[#F4D06F]" : ""}`}>
                      ...
                      <label htmlFor="right" className={`dp-card p-4 cursor-pointer ${form.placement_side === "RIGHT" ? "border-[#F4D06F]" : ""}`}>
                        <div className="flex items-center gap-3">
                    ...
                    <div className="mt-2 text-xs text-white/50">
                      Suggested: <span className="text-[#F4D06F]">{refInfo.referrer.suggested_side}</span> · Balance diff: {refInfo.referrer.balance_diff}
                    </div>
                  </div>
                )}
              </div>
              ...
              <div className="flex items-start gap-3 pt-2">
                ...
                <label htmlFor="terms" className="text-xs text-white/60 leading-relaxed">
                  I have read and agree to the Dreampick Private Limited terms & conditions, privacy policy, and payout schedule.
                </label>
              </div>
              ...
              <p className="text-center text-sm text-white/50">
                Already have an account? <Link to="/login" className="text-[#F4D06F] hover:underline" data-testid="register-login-link">Sign in</Link>
              </p>
            </form>
            ...
            <aside className="lg:col-span-2 space-y-4">
              <div className="dp-card p-6" data-testid="register-preview-card">
                <div className="overline text-[#F4D06F] mb-3">Placement Preview</div>
                {!refInfo?.valid && (
                ...
                {refInfo?.valid && preview && (
                  <div className="space-y-3 text-sm">
                    ...
                    <div className="flex justify-between border-b border-white/5 pb-2">
                      ...
                      <span className={preview.direct_slot_empty ? "text-[#F4D06F]" : "text-white/80"}>
                    ...
                    <div className="flex justify-between border-b border-white/5 pb-2">
                      ...
                      <span className="text-[#F4D06F]">{preview.placement_preview.placement_side}</span>
                    </div>
